...
class basic_vae(object):
    def __init__(self,_name='VAE',_xdim=784,_zdim=128,
                 _hdim=512,_actv=lrelu,
        ...
        self.build_graph()
        print ("[%s] Instantiated" % (self.name))
        print (" xdim:[%s] zdim:[%d] hdim:[%d]"%
               (self.xdim,self.zdim,self.hdim))
        """ Print """
        print ("Trainable Variables")
        for i in range(len(self.t_vars)):
            w_name  = self.t_vars[i].name
    ...
    def encoder(self,_x,_name='encoder',_reuse=False):
        with tf.variable_scope(_name, reuse=_reuse) as scope:
            with slim.arg_scope([slim.fully_connected],activation_fn=self.actv,
                ...
                _net = slim.fully_connected(_x,self.hdim,scope='lin0')
                _net = slim.fully_connected(_net,self.hdim,scope='lin1')
                _zmu = slim.fully_connected(_net,self.zdim,activation_fn=None,scope='zmu')
                _zlogvar = slim.fully_connected(_net,self.zdim,activation_fn=None,scope='zlogvar')
    ...
    def decoder(self,_z,_name='decoder',_reuse=False):
        with tf.variable_scope(_name, reuse=_reuse) as scope:
            with slim.arg_scope([slim.fully_connected],activation_fn=self.actv,
                                weights_initializer=self.fully_init,biases_initializer=self.bias_init,
                                normalizer_fn=slim.batch_norm, normalizer_params=self.bn_params):
                _net = slim.fully_connected(_z,self.hdim,scope='lin0')
                _net = slim.fully_connected(_net,self.hdim,scope='lin1')
                _net = slim.fully_connected(_net,self.xdim,scope='lin2')
        return _net
    """
        Build Model
    """
    def build_model(self):
        self.x = tf.placeholder(tf.float32,shape=[None,self.xdim])
        ...
        """ Encoder """
        self.zmu,self.zlogvar = self.encoder(self.x,_name='encoder',_reuse=False)
        """ Sample z """
        ...
        """ Sample with given z (for testing) """
        self.xrecon_sample = self.decoder(self.z,_name='decoder',_reuse=True)
        """ Reconstruct with given x """
        self.zmu_test,_ = self.encoder(self.x,_name='encoder',_reuse=True)
        self.xrecon_test = self.decoder(self.zmu_test,_name='decoder',_reuse=True)
        """ Get Variables """
    ...
    """
    def build_graph(self):
        self.l1_loss     = 1./2.*tf.norm(self.xrecon_train-self.x,ord=1,axis=1)
        self.l2_loss     = 1./2.*tf.norm(self.xrecon_train-self.x,ord=2,axis=1)
...
class basic_dcvae(object):
    ...
    def build_model(self):
        ...
        self.xrecon_sample = self.decoder(self.z,_name='decoder',_reuse=True)
        """ Reconstruct with given x """
        self.zmu_test,_ = self.encoder(self.x_reshape,_name='encoder',_reuse=True)
        self.xrecon_test = self.decoder(self.zmu_test,_name='decoder',_reuse=True)
    ...
    """
    def build_graph(self):
        _xrecon_train_reshpe = tf.reshape(self.xrecon_train,shape=[-1,self.xdim_flat])
        self.l1_loss     = 1./2.*tf.norm(_xrecon_train_reshpe-self.x,ord=1,axis=1)
        self.l2_loss     = 1./2.*tf.norm(_xrecon_train_reshpe-self.x,ord=2,axis=1)
        self.recon_loss  = tf.reduce_mean(self.l1_loss)
        self.kl_loss_raw = 0.5*tf.reduce_sum(tf.exp(self.zlogvar)+self.zmu**2-1.-self.zlogvar,1)
        self.kl_loss     = tf.reduce_mean(self.kl_loss_raw)
        self.total_loss  = self.recon_loss + self.kl_weight*self.kl_loss
